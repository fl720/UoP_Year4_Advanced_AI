...
% You need input vector x with 4 variables, e.g., x=[-13.0 3.0 12.0 15.5]
% The parameter m should be one of m = 5, 7, or 10.
% Run with: >> Shekel_fun(x,m)
...
function y = Shekel_fun(x,m)
 if ((length(x)==4) && (m==5 || m==7 || m==10))
    ...
    switch m
        case 5
            a=M(:,1:5); %first 5 columns of M 
            c=V(1:5);	%first 5 elements of V
        case 7
            a=M(:,1:7);	%first 7 columns of M
            c=V(1:7);	%first 7 elements of V
        case 10
            a=M;	%the whole matrix M
            c=V;	%the whole vector V
    end
    s1=0;
    for i = 1: m
        s2 = 0;
